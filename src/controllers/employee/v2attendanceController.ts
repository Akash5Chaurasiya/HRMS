    ...
    if (
      ...
      if (attendanceRecord) {
        ...
        if (shift === "night") {
          ...
          let attendanceId = atten[atten.length - 1]?._id;
          let dayRecords = await v2AttendanceModel.findById(attendanceId);
          let length = dayRecords?.punches ? dayRecords?.punches.length - 1 : 0;

          const dayPunch: any = attendanceRecord?.punches[length];
          const DayPunchOut: any = dayPunch.punchOut;
          if (DayPunchOut === null) {
          ...
          let attendanceId = atten[atten.length - 1]?._id;
          let nightRecords = await v2AttendanceModel.findById(attendanceId);
          let length = nightRecords?.punches
            ? nightRecords?.punches.length - 1
            : 0;
          const nightPunch: any = attendanceRecord?.punches[length];
          const NightPunchOut: any = nightPunch.punchOut;
        ...
        let atten = await v2AttendanceModel.find({
          employeeId: id,
          date: { $gte: date, $lt: nextDay },
          shift: oldShift,
        });
        let attendanceId = atten[atten.length - 1]?._id;
        let attendanceRecords: any = await v2AttendanceModel.findById(
          attendanceId
        );
        if (attendanceRecords) {
          let length = attendanceRecords?.punches
            ...
            : 0;
          const nightPunch: any = attendanceRecords.punches[length];
          const NightPunchOut: any = nightPunch.punchOut;
          if (NightPunchOut === null) {
...
export const updateAttendanceDetails = catchErrorAsync(
